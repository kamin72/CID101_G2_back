      ...
      <div class="button-wrap d-flex justify-content-between py-4">
        <div class="left-wrap d-flex justify-content-between">
          <button
            type="button"
            class="btn me-1"
            :class="{ 'btn-primary': showActive, 'btn-secondary': !showActive }"
            style="height: 40px"
    ...
    <form @submit.prevent="addUpdateDiscount" id="addDiscountform">
      ...
      <div class="mb-3">
        ...
        <input
          ...
          class="form-control"
          id="discountAmount"
          v-model="newDiscount.dis_amount"
        />
      </div>
      <div class="mb-3">
        ...
        <input
          ...
          class="form-control"
          id="discountTime"
          v-model="newDiscount.dis_set_date"
        />
      </div>
...
export default {
  ...
  computed: {
    ...
    filteredDiscounts() {
      ...
      return this.discount.filter((item) => {
        // 判断是否符合搜索条件
        const matchSearch =
          item.dis_name.toLowerCase().includes(search) || String(item.dis_amount).includes(search)
  ...
  methods: {
    ...
    deleteDiscount(item) {
      fetch(
        `${import.meta.env.VITE_API_URL}/discountManage/discountType_delete.php?dis_serial=${item.dis_serial}`,
        {
          method: 'GET'
        }
      )
