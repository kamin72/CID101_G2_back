    ...
    <div class="row py-5">
      ...
      <div class="button-wrap d-flex justify-content-between py-4">
        <div class="left-wrap d-flex justify-content-between">
          <button @click="searchMember(1)" type="button" class="btn me-1" :class="{'btn-primary': normal_button_action, 'btn-secondary': !normal_button_action}" style="height: 40px;">一般會員</button>
          <button @click="searchMember(2)" type="button" class="btn" :class="{'btn-primary': wholesaler_button_action, 'btn-secondary': !wholesaler_button_action}" style="height: 40px;">批發商會員</button>
          <button @click="searchMember(3)" type="button" class="btn" :class="{'btn-primary': review_button_action, 'btn-secondary': !review_button_action}" style="height: 40px;margin: 0 4px">待審核</button>
          <button @click="searchMember(4)" type="button" class="btn" :class="{'btn-primary': failed_button_action, 'btn-secondary': !failed_button_action}" style="height: 40px;margin: 0 4px">審核未通過</button>
        </div>
      ...
      <tbody>
        <tr v-for="member in paginatedMember" :key="member.no">
          ...
          <td class="d-flex gap-2">
            <button type="button" class="btn btn-primary"  @click="openMemberInfo(member)">詳情</button>
          </td>
        </tr>
      </tbody>
    </table>
      ...
      <form @submit.prevent="updateStatus" id="memberInfo" class="w-100 row">
        <div class="memberInfo col">
          <div class="mb-3 row">
                        ...
                        <div class="col-sm-8">
                          <input type="text" readonly class="form-control-plaintext" id="createDate" v-model="memberInfo.created_at" />
                        </div>
                    </div>
                    <div class="mb-3 row">
                        <label for="createDate" class="col-sm-4 col-form-label">修改日期</label>
                        <div class="col-sm-8">
                          <input type="text" readonly class="form-control-plaintext" id="updateDate" v-model="memberInfo.updated_at" />
                        </div>
                    </div>
                    <div class="mb-3 row">
                        <label for="memberNo" class="col-sm-4 col-form-label">會員編號</label>
                        <div class="col-sm-8">
                          <input type="text" readonly class="form-control-plaintext" id="memberNo" v-model="memberInfo.no" />
                        </div>
                    </div>
                    <div class="mb-3 row">
                        <label for="memberName" class="col-sm-4 col-form-label">姓名</label>
                        <div class="col-sm-8">
                          <input type="text" readonly class="form-control-plaintext" id="memberName" v-model="memberInfo.name" />
                        </div>
                    </div>
                    <div class="mb-3 row">
                    ...
                    <div class="mb-3 row">
                        ...
                        <div class="col-sm-8">
                          <input type="text" readonly class="form-control-plaintext" id="memberPhone" v-model="memberInfo.phone" />
                        </div>
                    </div>
                    <div class="mb-3 row">
        ...
        <div class="retailerInfo col" v-if="memberInfo.identity == 2">
          ...
          <div class="mb-3 row">
                    ...
                    <div class="mb-3 row w-100" v-if="memberInfo.identity == 1" >
                        ...
                        <div class="col-sm-8">
                          <select class="form-select" v-model="memberInfo.status" aria-label="Default select example" >
                    ...
                    <div class="d-grid gap-2 d-md-flex justify-content-md-center">
                        ...
                        <button type="button" class="btn btn-secondary col-6" @click="showMemberInfo = false">取消</button>
                    </div>
      </form>
    </div>
</template>
...
export default {
  ...
  computed: {
    ...
    if (this.normal_button_action) {
      filtered = filtered.filter(item => item.identity === 1 && (item.status === 1 || item.status === 2));
    } else if (this.wholesaler_button_action) {
      filtered = filtered.filter(item => item.identity === 2 && (item.status === 1 || item.status === 2));
    } else if (this.review_button_action) {
      filtered = filtered.filter(item => item.identity === 2 && item.status === 0);
    } else if (this.failed_button_action) {
      filtered = filtered.filter(item => item.identity === 2 && item.status === 3);
    }
    // 排序
