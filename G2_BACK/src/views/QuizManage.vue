  ...
  computed: {
    filteredQuiz() {
      ...
      return this.temp.filter((item) => {
        return (
          item.q_name.toLowerCase().includes(this.search) ||
          item.q_option_a.toLowerCase().includes(this.search) ||
          item.q_option_b.toLowerCase().includes(this.search)
        )
      })
    },
    paginatedQuiz() {
